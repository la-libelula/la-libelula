...
7. Si la dirección de la persona ("direccion"), código postal ("codigoPostal"), o el municipio/provincia ("municipio") son visibles (por ejemplo, en la parte trasera del DNI o en el documento), extráelos. MUY IMPORTANTE: El campo "municipio" debe corresponder al municipio de residencia/domicilio actual, NO al municipio del lugar de nacimiento. En la trasera del DNI español aparecen ambos por separado; asegúrate de extraer el que forma parte de la dirección de residencia. Respecto al "codigoPostal", ten en cuenta que el DNI español NO tiene impreso el código postal en la tarjeta física. Intenta deducir el código postal correcto a partir del municipio, la provincia y la calle. Si es una ciudad grande con múltiples códigos postales y no tienes certeza absoluta del código exacto para esa calle, deja el campo "codigoPostal" completamente vacío ("") para que el usuario lo introduzca manualmente. ¡NUNCA inventes un código postal erróneo!
